...
#else
static const uint32_t hs_key_map[] = {
	//let app to diff the long/short power key 
	//KEY(HS_PWR_K, KEY_POWER),
...
void (*msm_vbus_notify)(int) = NULL;
void msm_vbus_notify_by_hs_init(void (*callback)(int online), int init)
{
	if(init)
		msm_vbus_notify = callback; 
	else
		msm_vbus_notify = NULL;		
}
//end
