...
/*record how long the APP sleeps or keeps awake*/
	...
	(bool from_idle, uint32_t sleep_delay, uint32_t sleep_limit)
{
	...
	MSM_PM_DPRINTK(MSM_PM_DEBUG_SUSPEND | MSM_PM_DEBUG_POWER_COLLAPSE,
		...
		if(collapsed == 1)
		{
			...
			zte_need_2_prink_rpc_while_wakeup();// record the RPC id while RPC wakeup
		}
	MSM_PM_DPRINTK(MSM_PM_DEBUG_CLOCK, KERN_INFO,
...
#ifdef CONFIG_MSM_GPIO_WAKE
#include <linux/input.h>	// enable  BACK HOME MENU to wakeup
