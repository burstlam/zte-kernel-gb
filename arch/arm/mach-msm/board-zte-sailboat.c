...
static void lcdc_lead_gpio_init(void)
{
	...
	if (gpio_request(GPIO_LCD_PANEL_ID, "gpio_id"))        
		pr_err("failed to request gpio_id\n");
}
...
static uint32_t lcdc_gpio_table[] = {
	...
	GPIO_CFG(GPIO_LCD_PANEL_ID, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA),};     
#else
static int gpio_array_num[] = {
...
static struct i2c_board_info i2c_devices[] = {
     ...
     */
    //mt9v113 is register to  aux-i2c 
...
//mt9v113 is register to  aux-i2c udelay 40 -> 2
    ...
    switch (pwr_mode)
    {
        ...
        case MSM_CAMERA_PWRDWN_MODE:
        {
            /*
               ...
               */
            rc_cam_dvdd  = vreg_disable(vreg_cam_dvdd);
            rc_cam_iovdd  = vreg_disable(vreg_cam_iovdd); //close io vdd
            rc_cam_avdd  = vreg_disable(vreg_cam_avdd);
            if ((rc_cam_dvdd) || (rc_cam_avdd))
            {
                CCRT("%s: vreg_disable failed!\n", __func__);
 ...
 * For MT9V113: 0.3Mp, 1/11-Inch System-On-A-Chip (SOC) CMOS Digital Image Sensor
 */
//mt9v113 is register to  aux-i2c  change io av value 
...
//mt9v113 is register to  aux-i2c reset gpio 26 
...
#if 0
#ifdef CONFIG_CACHE_L2X0
	if (machine_is_msm7x27_surf() || machine_is_msm7x27_ffa() || machine_is_sailboat()) {
...
MACHINE_START(SAILBOAT, "sailboat")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io        = MSM_DEBUG_UART_PHYS,
	.io_pg_offst    = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
